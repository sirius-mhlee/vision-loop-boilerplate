...
아래 2026-09-12 검증의 v002 모델은 2 epochs만 학습해 기본 표시 임계값 0.5를 넘는 예측이 없었습니다.
...
작업 ID 시각은 UTC입니다. 위 ID는 과거 검증 기록이며 새 실행에서 재사용하지 않습니다.
자신의 `result.json`에 기록된 평가 작업 ID를 앞 절의 `--view` 명령에 넣습니다.
...
2026-09-12 실행 후 toy 폴더의 디스크 사용량은 약 1.9 GB였습니다. 이미지 8장 외에 두 모델의
