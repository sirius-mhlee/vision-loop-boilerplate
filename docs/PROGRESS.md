...
| 3. FiftyOne 검수 | 샘플 DB·브라우저 검증 완료 | Annotation Schema, 마스크 수정, 승인·재검수·제외, 승인 기록과 변경 감지 |
...
- 서버는 2초 간격으로 승인한 라벨·이미지·기록의 일치를 검사하고 변경 시 수정 중으로 돌립니다.
  서버가 꺼져 있는 동안의 변경은 다음 실행에서 검사합니다. 후속 릴리스에서 사용할
  `approved_annotation()`도 상태 문자열만으로 승인 데이터를 허용하지 않습니다.
- FiftyOne 1.21의 펼친 이미지 상태 표시는 operator의 reload 요청 후에도 이전 값이 남는
  경우가 있습니다. DB 변경은 성공하며 브라우저 새로고침 후 최신 상태가 보입니다.
  현재 검사 방식은 완료 이미지 전체를 읽으므로 대규모 데이터 성능은 후속 확인 대상입니다.

## 검증 결과

- 기본 테스트 82개 통과. 실제 DB·GPU 테스트 3개는 기본 실행에서 명시적으로 제외합니다.
- 별도 호스트 실행에서 FiftyOne 통합 테스트 2개 통과: 실제 등록, 다른 프로세스의 재등록,
