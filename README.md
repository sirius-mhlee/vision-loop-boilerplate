...
`seed`는 모델을 만들기 전에 Python·NumPy·PyTorch에 적용합니다. 다만 RF-DETR 1.8.2의
Albumentations 증강은 별도 난수 생성기를 사용하고 현재 wrapper가 여기에 seed를 전달하지
않습니다. GPU 연산의 결정적 실행도 강제하지 않으므로 같은 설정의 새 학습이나 중단 후 재개가
동일한 증강·가중치·점수를 만드는 것은 보장하지 않습니다. 기록된 데이터·설정·학습 상태를
복원하는 범위와 결과의 완전한 수치적 재현은 구분합니다.
...
재개용 체크포인트에는 옵티마이저·스케줄러·에포크·step·EMA와 Python·NumPy·PyTorch의 전역
난수 상태, 이전 최적 모델을 포함하므로 `best.pt`보다 큽니다. 같은 run에서는 새 체크포인트를
저장하고 참조 정보를 갱신한 뒤 이전 재개 파일을 정리합니다. 이전 run의 결과는 보존하며
여러 run 사이의 모델 중복 제거는
...
`ingest`, `autolabel`, `review`, `review-batch`, `review-audit`는 터미널에서 tqdm 한 줄을
갱신합니다. 단계가 바뀌면 같은 줄의 제목과 처리 기준을 교체하고, 해당 단계의 시간·속도를
새로 계산합니다. 별도 옵션은 필요하지 않습니다. tqdm은 기본 의존성이므로
`ingest --local-only`에서도 사용할 수 있고, 전체 lock의 기존 `tqdm==4.70.0`을 유지합니다.

| 명령 | 표시 기준 |
|---|---|
| `ingest` | 파일 탐색·등록은 전체 개수 없이 처리 파일 수·속도와 등록·중복·복구·실패 건수 표시. 이후 카탈로그 전체 이미지를 FiftyOne에 동기화하는 진행률 표시 |
| `autolabel` | 입력 준비·무결성 확인·모델 로딩 상태를 표시한 뒤, 고정된 대상의 처리 수와 실패·저장 결과 재사용 건수 표시 |
| `review` | 선택한 queue·limit에 해당하는 검수 준비 대상 수와 초기화·기존 정답 유지·예측 없음 건수 표시. 검수 화면을 열기 전에 종료 |
| `review-batch` | 미리보기 생성 또는 적용의 처리 수/전체 수 표시. 미리보기 오류, 적용 실패·건너뜀 건수를 별도 표시 |
| `review-audit` | 승인 무결성 검사 건수·속도·승인 무효화 건수 표시. 검사 중 승인 상태가 변할 수 있어 고정된 전체 수나 퍼센트는 표시하지 않음 |

자동 라벨링과 미리보기·적용은 저장된 처리 상태를 기준으로 재개하고, 전체 감사는 저장된
검사 건수부터 표시합니다. `ingest` 재실행은 입력 파일을 다시 확인하므로 0부터 시작합니다.
진행률을 위해 전체 파일 목록을 메모리에 모으거나 파일 수를 세려고 디렉터리를 한 번 더
탐색하지 않습니다. 화면 갱신 빈도도 제한합니다.

**100%는 해당 단계에서 대상 처리를 마쳤다는 뜻이며, 모두 성공했다는 뜻은 아닙니다.**
개별 오류나 건너뜀도 처리 건수에 포함될 수 있습니다. 성공 여부와 세부 건수는 최종 결과 및
작업 폴더의 `report.json`에서 확인합니다. 경고·오류 출력은 유지하며, 진행률의 출력 대상인
표준 오류(`stderr`)가 터미널이 아니면 진행률을 생략합니다. 검수 서버의 주기적 검사에서는
진행률을 반복해서 띄우지 않습니다.
...
`dev` 의존성만 설치한 환경에서도 기본 단위 테스트를 실행할 수 있습니다. 선택 패키지가
필요한 검사는 해당 패키지가 없으면 건너뛰며, 순수 변환·해시 검사는 그대로 실행합니다.
...
├── progress.py     # 터미널 한 줄 진행률, 단계 전환과 처리 건수
