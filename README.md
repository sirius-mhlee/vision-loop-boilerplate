...
`restore --version v001`은 **이미 완성된 버전의 데이터를 별도 캐시에 가져오는 명령**입니다.
릴리스 생성 작업을 이어가거나 새 버전 번호를 만드는 명령이 아닙니다.
...
빈 정답의 수동 승인 이미지는 포함하고 미검수·수정 중·제외 상태는 포함하지 않습니다.
...
필요하면 `dvc_remote`를 외장 디스크나 로컬에 마운트한 NAS 경로로 설정합니다. 공유 캐시와
...
Git 태그는 `dataset/v001`이며 현재 HEAD를 기반으로 데이터 메타데이터만 추가한 별도 커밋을
가리킵니다. 현재 브랜치·Git index·코드 수정은 그대로 유지합니다. 메타데이터는 태그 안의
...
Git 원격으로 태그를 보내는 작업은 자동 수행하지 않습니다. 다른 clone에서는 태그를 받아야
하고 DVC remote에도 접근할 수 있어야 복원됩니다. 복원 시 태그의 DVC 메타데이터를 별도
작업 폴더에서 `pull`하며 공유 캐시를 재사용합니다. 재복원도 체크섬 검증에는 시간이 듭니다.
